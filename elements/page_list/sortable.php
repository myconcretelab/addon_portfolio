...
if (count($tagsObject->tags) && (in_array('tag-sorting',$styleObject->classesArray) || in_array('keyword-sorting',$styleObject->classesArray)) ) :
	...
	if (in_array('tag-sorting',$styleObject->classesArray)) :
	  echo '<li><a href="#show-all" data-option-value="*" class="mdl-button mdl-js-button mdl-button--raised mdl-button--accent">' . t('show all') .'</a></li>';
	  foreach ($tagsObject->tags as $handle => $tag):
	  echo '<li><a class="mdl-button mdl-js-button mdl-button--raised" href="#' . $handle . '" data-filter=".' . $handle . '">' . $tag . '</a></li>';
	endforeach;
	endif;
